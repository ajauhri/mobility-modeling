...
class RRGSnapshot(object):
    """
    Class captures properties of a Ride Request Graph(RRG) for a single
    snapshot of time. The RRG is a directed graph where nodes represent
    granular geographic area (like 100 x 100 m^2 area can be represented
    by a node).
    """
    def __init__(self):
        ...
        self.n_edges = 0
        self.n_nodes = 0
        self.nxg = nx.Graph()  # nx.clustering() works for undirected graphs
        ...
        for i in range(len(S)):
            ...
            satisfy the following conditions:
                1) S[i, 0] >= lat_grids[src_lat_cell - 1]
                                and
                   S[i, 0] < lat_grids[src_lat_cell]
                2) S[i, 1] >= lng_grids[src_lng_cell - 1]
                                and
                   S[i, 1] < lng_grids[src_lng_cell]
            """
    ...
    def compute_nodes_and_edges(self):
        ...
        for s, ds in self.out_weights.items():
            self.n_edges += len(ds)
        self.n_nodes = len(np.union1d(
            list(self.source_nodes.keys()),
