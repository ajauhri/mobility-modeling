...
def compute_stats(P, D, reqs_ts, reqs_over_time, args, params):
    ...
    for node_len in range(args.min_node_len, args.max_node_len+1, 50):
        ...
        for t, idxs in reqs_over_time.items():
            if len(idxs) <= 10 or (args.skip_night_hours and \
            ...
            if len(n_nodes) == 36:
                ...
                print('***', t, p)
                print('theory', theor_deg_exp)
                print('real', degree_exp, len(degree_exp))
            ...
            degree_mle_exp = 0
            if len(rrg_t.in_degree) >= 2:
                degree_mle_exp = helpers.real_degree_exp(rrg_t)
                degree_exp.append(degree_mle_exp)
            else:
                degree_exp.append(0)

            logging.debug(
                """node len={}m, time_bin={}, num_nodes={}, """
                """num_edges={}, degree_params={}, """
                """#rides={}""".format(
                    node_len, t, n_nodes[-1], n_edges[-1],
                    degree_mle_exp, len(idxs))
            )
